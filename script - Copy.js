...
// সব ডেটা লোড করা - FIXED VERSION
function loadAllData() {
    try {
        console.log('📂 Loading all data...');
        
        // ১. Individual data load করুন
        const savedSettings = localStorage.getItem('desco_settings');
        const savedTariffRates = localStorage.getItem('desco_tariffRates');
        const savedMeterInfo = localStorage.getItem('desco_meterInfo');
        const savedAutoBackup = localStorage.getItem('desco_autoBackup');
        
        // ১.১ Meters load করুন (loadMeterInfo তেও এটি করা হয়, তবে এখানেও রাখা ভালো)
        const savedMeters = localStorage.getItem('desco_meters');
        const savedActiveId = localStorage.getItem('desco_active_meter_id');
        ...
        if (savedMeters) {
            meters = JSON.parse(savedMeters) || [];
            activeMeterId = savedActiveId || (meters[0] ? meters[0].id : null);
        } else if (savedMeterInfo) {
             // Legacy support if meters not saved but meterInfo exists
             // loadMeterInfo will handle reconstruction better, so we can defer or just do minimal here
        }

        // ২. Core balance data load করুন
        const savedCurrentBalance = localStorage.getItem('desco_currentBalance');
        const savedTotalRecharge = localStorage.getItem('desco_totalRecharge');
        const savedTotalExpended = localStorage.getItem('desco_totalExpended');
        
        // ৩. Global variables সেট করুন (recalculate নয়)
        if (savedCurrentBalance) currentBalance = parseFloat(savedCurrentBalance);
        if (savedTotalRecharge) totalRecharge = parseFloat(savedTotalRecharge);
        if (savedTotalExpended) totalExpended = parseFloat(savedTotalExpended);
        
        // ৪. যদি কোনো value missing থাকে, শুধু তখনই calculate করুন
        if (!savedCurrentBalance || !savedTotalRecharge || !savedTotalExpended) {
            console.log('⚠️ Missing balance data, recalculating...');
            const electricityBills = transactions.filter(t => t.type === 'electricity_bill');
            const calculatedExpended = electricityBills.reduce((sum, bill) => sum + (bill.amount || 0), 0);
            
            if (!totalExpended || totalExpended === 0) {
                totalExpended = calculatedExpended;
            }
            
            if (!currentBalance || currentBalance === 0) {
                currentBalance = totalRecharge - totalExpended;
            }
        }
...
// মাসিক রিচার্জ যোগ করুন - নতুন ভার্সন
...
function updateProgressBar() {
    ...
    const maxBalance = Math.max(totalRecharge, currentBalance, 1000);
...
function updateDashboard() {
    try {
        ...
        console.log('✅ Safe dashboard update completed');
...
function updateBalance() {
    // অথেন্টিকেশন চেক (লগিন সিস্টেমের জন্য)
    if (!checkAuthentication()) {
        showLoginModal();
        return;
    }
    ...
    try {
        ...
        if (difference < 0) {
            ...
            const bill = {
                ...
                description: `বিদ্যুৎ বিল - ${expenseAmount.toFixed(2)} টাকা (${estimatedUnits.toFixed(2)} kWh) - ${formatTimestampForDisplay(new Date(date).toISOString())}`,
            ...
            transactions.unshift(bill); // নতুন ট্রানজেকশন শুরুতে যোগ করুন
            totalExpended += expenseAmount;
            
            showNotification(`✅ ব্যালেন্স আপডেট করা হয়েছে! বিদ্যুৎ বিল: ${expenseAmount.toFixed(2)} টাকা`, 'success');
            ...
            transactions.unshift(recharge); // নতুন ট্রানজেকশন শুরুতে যোগ করুন
            totalRecharge += rechargeAmount;
            
            showNotification(`✅ ব্যালেন্স আপডেট করা হয়েছে! রিচার্জ: ${rechargeAmount.toFixed(2)} টাকা`, 'success');
        }
...
// মাসিক রিচার্জ যোগ করার ফাংশন - সর্বশেষ ব্যালেন্স আপডেট করার জন্য
function addMonthlyRecharge() {
    try {
        const amount = parseFloat(document.getElementById('rechargeAmount').value);
        const date = document.getElementById('rechargeDate').value;
        ...
        monthlyRecharges.push(recharge);
        
        // বর্তমান ব্যালেন্স আপডেট করুন (শুধু ব্যবহারযোগ্য অংশ)
        currentBalance += billDetails.energyCost;
        totalRecharge += amount;
        
        // ট্রানজেকশন হিস্ট্রিতে যোগ করুন
        transactions.push({
            id: Date.now(),
            type: 'recharge',
            amount: amount,
        ...
        let message = `✅ DESCO রিচার্জ যোগ করা হয়েছে!\n\nমোট রিচার্জ: ${amount} টাকা\nব্যবহারযোগ্য: ${billDetails.energyCost.toFixed(2)} টাকা`;
        if (billDetails.includeDemandCharge) {
            message += `\nডিমান্ড চার্জ: ${billDetails.demandCharge} টাকা`;
        }
        message += `\nভ্যাট: ${billDetails.vat.toFixed(2)} টাকা\nরিবেট: ${billDetails.rebate.toFixed(2)} টাকা`;
...
// বর্তমান মিটারের ডেটা লোড করুন - করেক্টেড
...
async function backupToGoogleDrive(backupData) {
    // Google Drive API ইন্টিগ্রেশন
    showNotification('📁 গুগল ড্রাইভে ব্যাকআপ করা হচ্ছে...', 'info');
    
    // 임시 구현 - actual API call এর জন্য
    return new Promise((resolve) => {
        setTimeout(() => {
            const backupId = 'gd_' + Date.now();
            localStorage.setItem(`google_drive_backup_${backupId}`, JSON.stringify(backupData));
            resolve({ success: true, backupId, provider: 'Google Drive' });
        }, 2000);
    });
}
...
async function backupToDropbox(backupData) {
    // Dropbox API ইন্টিগ্রেশন
    showNotification('📦 ড্রপবক্সে ব্যাকআপ করা হচ্ছে...', 'info');
    
    // 임시 구현 - actual API call এর জন্য
    return new Promise((resolve) => {
        setTimeout(() => {
            const backupId = 'db_' + Date.now();
            localStorage.setItem(`dropbox_backup_${backupId}`, JSON.stringify(backupData));
            resolve({ success: true, backupId, provider: 'Dropbox' });
        }, 2000);
    });
}
